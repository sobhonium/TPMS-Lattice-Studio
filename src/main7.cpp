...
ofstream coordinat("MinMaxCoordinates.txt"); 
...
void SaveGenerators(GyroidSkeletal skelet){
    ...
    ofstream generator("generators.txt"); 
...
ofstream outdata; outdata.open("demofile.txt");


// cylinder check
cout<<"Bound: "<<boundx<<" "<<boundy<<" "<<boundz<<endl;
 if (is_surf==1){
    for (double i=-1;i<=1; i+= 1*stepsize)
      for (double j=-1;j<=1; j+=1*stepsize)
        for (double k=-3;k<3; k+=1*stepsize){
    ...
    for (double i=-1;i<=1; i+= 3*stepsize)
      for (double j=-1;j<=1; j+=3*stepsize)
        for (double k=-3;k<3; k+=3*stepsize){
